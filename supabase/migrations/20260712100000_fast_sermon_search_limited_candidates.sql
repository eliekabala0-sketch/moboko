-- Hard-limit paragraph candidates before joins/ranking to avoid statement timeouts.
...
exact_phrase_candidates as (
  select p.sermon_id, p.paragraph_number, p.paragraph_text
  from q_input qi
  join public.sermon_paragraphs p
    on qi.important_query <> ''
   and p.search_tsv @@ phraseto_tsquery('french', qi.important_query)
   and position(qi.query_norm in p.normalized_text) > 0
  limit (select branch_lim from q_input)
),
exact_phrase_hits as (
  select s.slug, s.title, s.year, s.preached_on, s.location, c.sermon_id, c.paragraph_number, c.paragraph_text,
    1 as relevance_tier, 8::real as search_rank
  from exact_phrase_candidates c
  join base_sermons s on s.id = c.sermon_id
),
websearch_candidates as (
  select p.sermon_id, p.paragraph_number, p.paragraph_text, max(ts_rank_cd(p.search_tsv, websearch_to_tsquery('french', q.q)))::real as rank
  from queries q
  join public.sermon_paragraphs p on p.search_tsv @@ websearch_to_tsquery('french', q.q)
  group by p.sermon_id, p.paragraph_number, p.paragraph_text
  limit (select branch_lim from q_input)
),
websearch_hits as (
  select s.slug, s.title, s.year, s.preached_on, s.location, c.sermon_id, c.paragraph_number, c.paragraph_text,
    2 as relevance_tier, c.rank as search_rank
  from websearch_candidates c
  join base_sermons s on s.id = c.sermon_id
),
important_word_candidates as (
  select p.sermon_id, p.paragraph_number, p.paragraph_text,
    ts_rank_cd(p.search_tsv, plainto_tsquery('french', qi.important_query))::real as rank
  from q_input qi
  join public.sermon_paragraphs p
    on qi.important_query <> ''
...
important_word_hits as (
  select s.slug, s.title, s.year, s.preached_on, s.location, c.sermon_id, c.paragraph_number, c.paragraph_text,
    3 as relevance_tier, c.rank as search_rank
  from important_word_candidates c
  join base_sermons s on s.id = c.sermon_id
),
near_expression_candidates as (
  select p.sermon_id, p.paragraph_number, p.paragraph_text,
    similarity(p.normalized_text, qi.query_norm)::real as rank
  from q_input qi
  join public.sermon_paragraphs p
    on char_length(qi.query_norm) >= 8
   and p.normalized_text % qi.query_norm
  limit (select branch_lim from q_input)
),
near_expression_hits as (
  select s.slug, s.title, s.year, s.preached_on, s.location, c.sermon_id, c.paragraph_number, c.paragraph_text,
    4 as relevance_tier, c.rank as search_rank
  from near_expression_candidates c
  join base_sermons s on s.id = c.sermon_id
),
unioned as (
