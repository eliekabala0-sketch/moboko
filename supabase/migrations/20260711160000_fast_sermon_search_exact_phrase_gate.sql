-- Make exact phrase branch use the GIN FTS index before substring verification.
...
with input as (
  select
    ...
    least(320, greatest(60, greatest(1, least(100, coalesce(p_limit, 20))) * 8)) as branch_lim,
...
q_input as (
  select t.*, array_to_string(t.important_words[1:10], ' ') as important_query
  from terms t
),
queries as (
...
exact_phrase_hits as (
  ...
  from q_input qi
  join base_sermons s on char_length(qi.query_norm) >= 3 and qi.important_query <> ''
