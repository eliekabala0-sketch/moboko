-- Replace the first fast search function with index-friendly bounded branches.
...
with input as (
  select
    ...
    least(600, greatest(80, greatest(1, least(100, coalesce(p_limit, 20))) * 12)) as branch_lim,
...
q_input as (
  select
    t.*,
    array_to_string(t.important_words[1:10], ' ') as important_query
  from terms t
),
queries as (
  ...
  cross join lateral unnest(
    case
      when p_queries is not null and array_length(p_queries, 1) > 0
        then array_prepend(qi.query_raw, p_queries)
...
base_sermons as (
  ...
  where s.is_published = true
    ...
    and (
      qi.title_filter is null
      or s.title ilike '%' || replace(replace(replace(qi.title_filter, '\', '\\'), '%', '\%'), '_', '\_') || '%' escape '\'
    )
    and (
      qi.location_filter is null
      or s.location ilike '%' || replace(replace(replace(qi.location_filter, '\', '\\'), '%', '\%'), '_', '\_') || '%' escape '\'
    )
),
...
exact_phrase_hits as (
  select
    ...
    p.sermon_id, p.paragraph_number, p.paragraph_text,
    1 as relevance_tier,
    8::real as search_rank
  from q_input qi
  join base_sermons s on char_length(qi.query_norm) >= 3
  join public.sermon_paragraphs p
    on p.sermon_id = s.id
   and p.normalized_text ilike '%' || replace(replace(replace(qi.query_norm, '\', '\\'), '%', '\%'), '_', '\_') || '%' escape '\'
...
websearch_hits as (
  select
    ...
    p.sermon_id, p.paragraph_number, p.paragraph_text,
    2 as relevance_tier,
    ts_rank_cd(p.search_tsv, websearch_to_tsquery('french', q.q))::real as search_rank
  from queries q
  join base_sermons s on true
  join public.sermon_paragraphs p
    on p.sermon_id = s.id
   and p.search_tsv @@ websearch_to_tsquery('french', q.q)
...
important_word_hits as (
  select
    ...
    p.sermon_id, p.paragraph_number, p.paragraph_text,
    3 as relevance_tier,
    ts_rank_cd(p.search_tsv, plainto_tsquery('french', qi.important_query))::real as search_rank
  from q_input qi
  join base_sermons s on qi.important_query <> ''
  join public.sermon_paragraphs p
    on p.sermon_id = s.id
   and p.search_tsv @@ plainto_tsquery('french', qi.important_query)
  order by search_rank desc
  limit (select branch_lim from q_input)
),
near_expression_hits as (
  select
    ...
    p.sermon_id, p.paragraph_number, p.paragraph_text,
    4 as relevance_tier,
    similarity(p.normalized_text, qi.query_norm)::real as search_rank
  from q_input qi
  join base_sermons s on char_length(qi.query_norm) >= 8
  join public.sermon_paragraphs p
    on p.sermon_id = s.id
   and p.normalized_text % qi.query_norm
  order by search_rank desc
  limit (select branch_lim from q_input)
),
unioned as (
...
deduped as (
  select
    slug, title, year, preached_on, location, sermon_id, paragraph_number, paragraph_text,
    min(relevance_tier) as relevance_tier,
...
left join public.sermon_paragraphs prev_p
  on prev_p.sermon_id = r.sermon_id and prev_p.paragraph_number = r.paragraph_number - 1
left join public.sermon_paragraphs next_p
  on next_p.sermon_id = r.sermon_id and next_p.paragraph_number = r.paragraph_number + 1
order by
